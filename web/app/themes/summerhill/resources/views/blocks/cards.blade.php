...
<section class="cards w-full">
  <div class="container pt-16 pb-10 @if($introduction_text) sm:pt-24 sm:pb-16 @else sm:py-16 @endif flex flex-col">
    ...
    <h2
      class="text-black font-souvenir font-medium leading-average text-center text-lg sm:text-h6 md:text-h5 w-full sm:w-3/4 mx-auto">
      {{ $introduction_text }}
    </h2>
    @endif

    <div class="mt-0 @if($introduction_text) sm:mt-16 md:mt-24 @endif flex flex-col md:flex-row md:justify-between">
      ...
      <div
        ...
        @if($card['title'])
        <h5 class="font-souvenir text-h5 font-normal leading-average text-black md:mt-8">{{ $card['title'] }}</h5>
        @endif
        @if($card['text'])
        <div
          class="mt-4 @if($card['title']) md:mt-4 @else md:mt-10 @endif text-black text-sm md:text-base font-roboto font-normal md:leading-loose leading-medium">
          {!! $card['text']
          !!}</div>
        @endif
      </div>
