...
<section
  ...
  <div class="text_button__content container text-center w-5/6 xl:{{ $width }}">
    @if($heading)
    <h2
      class="text-black font-souvenir @if($is_heading_big) text-h4 sm:text-h4 md:text-h3 lg:text-h2 @else text-lg md:text-h6 lg:text-h5 @endif font-medium leading-normal">
      {{ $heading }}
    </h2>
    @endif
    @if($subheading)
