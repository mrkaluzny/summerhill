...
<section class="relative w-full flex justify-center items-center">
  <div class="text_button @if($has_background_color && $default_color) bg-grey-lightest @endif"
    ...
    <div class="text_button__content container text-center w-5/6 xl:{{ $width }}">
      @if($heading)
      <h2
        class="text-black font-souvenir @if($is_heading_big) text-h4 sm:text-h4 md:text-h3 lg:text-h2 @else text-lg md:text-h6 lg:text-h5 @endif font-medium leading-average">
        {!! $heading !!}
      </h2>
      @endif
      @if($subheading)
      <h3 class="text-grey-darkest font-roboto text-sm md:text-base font-normal mt-6 leading-loose">
        {{ $subheading }}
      </h3>
      @endif
      @if($button)
